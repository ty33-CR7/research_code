...
for i in range(N):
    ...
    out_dir = os.path.abspath(os.path.join(BASE_DIR, "..", "data", "external", "dist", f"epsilon{epsilon}",f"{i}"))
    os.makedirs(out_dir, exist_ok=True)
    user_process_DR(X,domain_dict,y,epsilon,d,out_dir)


    #収集者側の処理
    file_path=f"../data/external/domain/epsilon{epsilon}/{i}/ADR_domain_T_{low_threshold}_L_{max_interval_len}.csv"
    # 各属性に対して処理を実行
    dict_domain={}
    for column in adult_column:
        ...
        cross_table = pd.read_csv(f"../data/external/dist/epsilon{epsilon}/{i}/{column}_OUE_estimation.csv",index_col=0)
    ...
    os.makedirs(f"../data/external/domain/epsilon{epsilon}/{i}", exist_ok=True)
