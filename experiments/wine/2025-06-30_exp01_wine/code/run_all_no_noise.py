...
#前処理フェーズ
preprocess_wine_quality_data(bins=L)  # rawから処理
...
for epsilon in ["no_noise"]: 
    ...
    for fold_index,(train_index,test_index) in enumerate(skf.split(X,y),start=1):
        ...
        for noise_num in range(N):
            ...
            out_dir = os.path.abspath(os.path.join(BASE_DIR, "..", "data", "external", "dist", f"epsilon{epsilon}",f"Fold{fold_index}",f"{noise_num}"))
            ...
            file_path = os.path.join(
                ...
                f"epsilon{epsilon}", f"Fold{fold_index}", f"{noise_num}",
            ...
            model_out_dir = os.path.abspath(os.path.join(BASE_DIR, "..", "models", f"epsilon{epsilon}",f"Fold{fold_index}",f"{noise_num}"))
